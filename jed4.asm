; This is JED.COM the text editor.  **** Modified, search for "-jedmod"  ****
...
main_program:
	...
	ld a, 48
    ld (VIEW_HEIGHT),a   ;Increase rows from 24 to 48  -jedmod
	
    ; Read in JED.KEY if it exists
    call read_jed_keys   ;Does not presently change screen size 
...
show_current_line_simple:     
    and 01111111b                   ; Clear bit 7 - was %01111111 -jedmod
    call print_a
    ld a, (current_col)
    inc a
    ld (current_col), a
...
make_uppercase:
    ...
    cp 'z'+1
    ret nc 
    and 11011111b    ;was %11011111 -jedmod
    ret
...
all_done:
    db 0
keybuff:
    dc 10,0      ;was ds 10 - fill with 0s -jedmod
keycounter:
    db 0
keypointer:
    dw 0
...
filename_buffer:
    ds 15,0     ;was ds 15 - fill with 0s -jedmod
stack:
    ds 31,0     ;was ds 31 - fill with 0s -jedmod
stacktop:
    db 0
...
rename_temp_to_original_file:
	...
	cp 255
	jr nz, ren_rtn
    ld de, failed_to_rename_message
    call show_string_de	
ren_rtn:
    ret
	
failed_to_rename_message:
    db 'ERROR renaming file!',13,10,'$'
...
erase_original_file:
    ...
    call BDOS
;	cp 255                       ; temp debug -jedmod
;	jr nz, del_rtn               ;
;   ld de, failed_to_delete_msg  ;
;   call show_string_de		     ; temp debug -jedmod
del_rtn:	
    ret
	
;  added error messages for debug purposes -jedmod	
failed_to_delete_msg:
    db 'ERROR deleting original file!',13,10,'$'	
;failed_to_delete_msg1:
;    db 'ERROR deleting temp file!',13,10,'$'
failed_to_create_msg:
    db 'ERROR creating temp file!',13,10,'$'
...
clear_fcb:	
 ...
 ;   ret nz
    cp 255
	jr nz, set_DMA
    ld de, failed_to_create_msg     ;debug -jedmod
    call show_string_de		
	ret
set_DMA:
    ld de, DMA
...
sector_complete:
    ...
    call BDOS	
	cp 0	     ; 0=success   -jedmod
;    cp 255      ; *** possible error ***, manual specifies 0 or "non zero"
;    jr nz, sector_complete_end
	jr z, any_more                     ; added for debug -jedmod
	ld de, failed_to_write_seq_msg     ;
    call show_string_de	               ;
	jr sector_complete_end             ;
failed_to_write_seq_msg:               ; added for debug -jedmod
    db 'ERROR failed sequential write!',13,10,'$'
...
sector_complete_end:
    ...
    ld de, FCB
    call BDOS
	cp 255                           ; added for debug -jedmod
	jr nz, complete_rtn              ;
	ld de, failed_to_close_file_msg  ;
    call show_string_de		         ; added for debug -jedmod
complete_rtn:	
    ret
failed_to_close_file_msg:
    db 'ERROR failed to close file!',13,10,'$'	
...
load_file_done:
	...
	cp 255                                  ; added for debug -jedmod
	jr nz, file_closed                      ;
    ld de, could_not_close_input_file_msg   ;
    call show_string_de                     ;
file_closed:	
    ld hl, (doc_end)
    ld (hl), END_OF_TEXT
...
could_not_close_input_file_msg:
    db 'File failed to close input file',13,10,'$'
